...
def viterbi(V, E):
    # O(V+E), LIS, TSP, knapsacks, MIS
    order = topological(V, E)
    if order == None:
        return False
        
    # O(E), create edge relationship
    ...
    # O(VE), search longest path
    for node in order: # V
        for next_node in edge[node]: # E
            if opt[next_node] < opt[node]+1:
                opt[next_node] = opt[node]+1
                back[next_node] = node
    node = -1
    rs = []
    while node in back:
